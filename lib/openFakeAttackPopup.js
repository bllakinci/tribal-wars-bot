...
function startAttack() {
  try {
    ...
    selectedVillages.map(x => {
      setTimeout(() => {
        var targetVillageId = x.getAttribute("villageId");
        ...
        for (var i = 0; i < attackCount; i++) {
          if (hasError === false) {
            var confirmAction = sendAttack(queryParams.get('village'), targetVillageId, targetX, targetY, units, (msg) => { hasError = true; $('#fake-attack-error-text').text(`* ${msg}`) });
            if (confirmAction)
